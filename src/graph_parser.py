...
def parse(keypoints, path, img_res, node_type, filename, cli):
    """
    Given a set of pose estimated keypoints, parses those points, and
    returns a list of Networkx augmented graphs containing those points,
    if the cli flag is False, otherwise returns None.
    :param keypoints: set of keypoints generated by OpenPose.
    :param str path: path where the output file should be saved
    :param list img_res: resolution of the given frame/images. Idx 1
    represents the x, idx 1 represents the y.
    :param dict node_type: dictionary of flags that represents the type
    of nodes for each keypoint
    :param bool cli: a flag for indicating the pipeline usage: cli or
    module
    ...
    if path is not None:
        ...
        with open(json_file, 'w') as out_file:
            ujson.dump(graph, out_file)
    if not cli:
        return get_nx_graphs(graph, node_type)
    else:
        return None
