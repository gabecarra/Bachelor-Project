...
def __generate_openpose_dict(keypoints, node_type: list) -> dict:
    """
    ...
    :param list node_type: list of node types. Can be one of 'pose', 'hand'
    or 'face'
    ...
    for i in range(n_person):
        ...
        n = 0 if 'face' not in node_type else 1
        if 'face' in node_type:
            person['face_keypoints_2d'] = keypoints[1][i].flatten().tolist()
        if 'hand' in node_type:
            person['hand_left_keypoints_2d'] = \
                keypoints[1 + n][i].flatten().tolist()
            person['hand_right_keypoints_2d'] = \
                keypoints[2 + n][i].flatten().tolist()
        keypts_dict['people'].append(person)
    return keypts_dict


def points_to_json(keypoints, path: str, img_res: str, node_type: list,
    ...
    :param str path: path of the file processed by OpenPose
    :param str img_res: resolution of the given frame/images
    :param list node_type: list of node types. Can be one of 'pose', 'hand'
    or 'face'
    :param str filename: name of the given file processed by OpenPose
    :param int index: Represents a counter for the frames of a video, if the
    file processed is a video, is an integer, otherwise None
    """
    try:
        os.mkdir(path)
    except FileExistsError:
