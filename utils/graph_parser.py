...
def __parse_nodes(graph: dict, keypoints: list, index: int, k_type: str) -> list:
    ...
    following attributes: {x, y, confidence}. Appends each node to the given graph.
...
def __parse_graph(open_pose_data: dict, filename: str, path: str) -> dict:
    """
    Given an OpenPose dataset_info, the codename of the file, and the path to the dataset_info returns the
    dataset_info parsed as a graph(JSON)
