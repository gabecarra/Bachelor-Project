...
def __normalize_data(A, X, y):
    ...
    :param numpy array A: numpy array of adj matrices
    :param numpy array X: numpy array of matrix-predictors
    :param numpy array y: list of outputs
    :return 3x numpy arrays: A_norm, X_norm and y_norm normalized
    """
    fil = np.full(y.shape[0], fill_value=True)
    for i in range(y.shape[0]):
        fil[i] = (X[i, :, 2] > .7).mean() > .5
    A_norm, X_norm, y_norm = A[fil], X[fil], y[fil]
    X_norm = X_norm[:, :, :2]
    X_norm -= X_norm.mean(axis=1, keepdims=True)
    X_norm /= X_norm.std(axis=1, keepdims=True)
    return A_norm, X_norm, y_norm
...
def generate_data():
    ...
    print('done')
    return __normalize_data(A, X, y)


def plot_results(fit_res):
    """
    Plots accurcy and loss for both train and test datasets.
...
A_train, A_test, x_train, x_test, y_train, y_test = train_test_split(A, X, y, test_size=0.2, random_state=13)
